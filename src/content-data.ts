...
function parse<T extends object>(entry: RawEntry, dates: string[] = []) {
  ...
  return { id: entry.id, data: parsed.data as T, body: parsed.body }
}

const notes: NoteEntry[] = [parse<NoteEntry['data']>({ id: 'learning-to-notice', raw: noteOne }, ['date']), parse<NoteEntry['data']>({ id: 'why-i-keep-a-small-site', raw: noteTwo }, ['date'])]
const ideas: IdeaEntry[] = [parse<IdeaEntry['data']>({ id: 'a-window-seat', raw: ideaOne }, ['date']), parse<IdeaEntry['data']>({ id: 'keep-the-receipt', raw: ideaTwo }, ['date']), parse<IdeaEntry['data']>({ id: 'the-useful-detour', raw: ideaThree }, ['date'])]
